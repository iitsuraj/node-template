...
const key = 'schedule-event';
// const messageQueue = new MessageQueueHelpers();
const jobSchedule = new JobScheduleHelpers();

// messageQueue.publishMessage(key, 'default message queue event for schedule');

jobSchedule.scheduleRecurringJob('schedule job-schedule definition', '1 minute', function () {
    console.warn('please implement schedule job-schedule here', new Date());
});

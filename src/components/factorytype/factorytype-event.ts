...
const key = 'factorytype-event';
// const messageQueue = new MessageQueueHelpers();
const jobSchedule = new JobScheduleHelpers();

// messageQueue.publishMessage(key, 'default message queue event for factorytype');

jobSchedule.scheduleRecurringJob('factorytype job-schedule definition', '1 minute', function () {
    console.warn('please implement factorytype job-schedule here', new Date());
});
